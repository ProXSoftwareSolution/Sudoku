...
// 'sudoku' is the name of this angular module example (also set in a <body> attribute in index.html)
// the 2nd parameter is an array of 'requires'
angular.module('sudoku', ['ionic'],function ($interpolateProvider) {
...
.controller('SudokuController', function($scope,$ionicModal,$ionicPopup,data) {
  ...
  $scope.generate = function() {    
    ...
    if(results['state']){
      ...
      $ionicPopup.alert({
                      title: 'Success',
                      content: 'Creating New Grid of Sudoku!'
                    });
  ...
  $scope.solve = function() {
    ...
    if(results['state']){
      ...
      $ionicPopup.alert({
                      title: 'Success',
                      content: 'Successfully solved the problem!'
                    });
    }
    else
      $ionicPopup.alert({
                      title: 'Failed',
                      content: 'Sorry can\'t be solved the problem!'
                    });
  };
})

.filter('column_modulo_3', function() {
    return function(input) {
      return input%3==1 ? 'margin_left_60' : '';
    };
    })
    
.filter('row_modulo_3', function() {
    return function(input) {
      return (input%3==0 && input<9) ? 'margin_bottom_60' : '';
    };
    })
    
.filter('check_number', function() {
    return function(input) {
      if (input >= 1 && input <= 9)       
        return input;
    };
